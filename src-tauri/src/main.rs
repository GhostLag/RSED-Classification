...
fn classification(address: &str) -> String {
    ...
    match address_json {
        ...
        Err(_) => format!("Enter a valid address"),
    }
...
async fn sa1_request() -> String {
    ...
    sa1.get("geographyList").unwrap()[17]["code"]
        .as_str()
        .unwrap_or("Error")
        .to_string()
}
...
fn sa1_lookup(sa1: u64) -> Option<u8> {
    let sa1_json: serde_json::Value =
        serde_json::from_str(&fs::read_to_string("../sa1.json").expect("File should exist?"))
            .unwrap();
...
#[tokio::main]
async fn main() {
    // let res: u64 = u64::from_str_radix(&sa1_request().await, 10).unwrap();
